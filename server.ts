...
async function startServer() {
  ...
  app.post("/api/gemini/search", async (req, res) => {
    try {
      ...
      res.status(500).json({ error: error.message || "Failed to parse or search items using Gemini AI." });
    }
  });
  ...
  app.post("/api/gemini/item-info", async (req, res) => {
    try {
      ...
      console.error("[Server Gemini Item-Info Error]:", error);
      res.status(500).json({ error: error.message || "Failed to generate item info." });
    }
  });
  ...
  app.post("/api/gemini/reviews", async (req, res) => {
    try {
      ...
      console.error("[Server Gemini Reviews Error]:", error);
  ...
  app.post("/api/gemini/recs", async (req, res) => {
    try {
      ...
      console.error("[Server Gemini Recs Error]:", error);
